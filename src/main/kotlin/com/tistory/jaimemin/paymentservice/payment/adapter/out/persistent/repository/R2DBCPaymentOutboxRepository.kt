...
import com.tistory.jaimemin.paymentservice.common.objectMapper
import com.tistory.jaimemin.paymentservice.payment.adapter.out.stream.util.PartitionKeyUtil
import com.tistory.jaimemin.paymentservice.payment.adapter.out.util.MySQLDateTimeFormatter
...
class R2DBCPaymentOutboxRepository(
    ...
    override fun insertOutbox(command: PaymentStatusUpdateCommand): Mono<PaymentEventMessage> {
        ...
        return databaseClient.sql(INSERT_OUTBOX_QUERY)
            ...
            .bind("partitionKey", paymentEventMessage.metadata["paritionKey"] ?: 0)
    ...
    private fun createPaymentEventMessage(command: PaymentStatusUpdateCommand): PaymentEventMessage {
        return PaymentEventMessage(
            ...
            metadata = mapOf(
                "paritionKey" to partitionKeyUtil.createPartitionKey(command.orderId.hashCode())
            )
        )
    }
