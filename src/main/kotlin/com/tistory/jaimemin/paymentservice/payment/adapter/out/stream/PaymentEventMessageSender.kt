package com.tistory.jaimemin.paymentservice.payment.adapter.out.stream

import com.tistory.jaimemin.paymentservice.common.Logger
import com.tistory.jaimemin.paymentservice.common.StreamAdatper
...
class PaymentEventMessageSender(
    ...
    fun receiveSendResult(results: SenderResult<String>) {
        if (results.exception() != null) {
            Logger.error(
                "sendEventMessage",
                results.exception().message ?: "receive an exception for event message send.", results.exception()
            )
        }
    ...
    fun handleSendResult() {
        sendResult.asFlux()
            ...
            .onErrorContinue { err, _ ->
                Logger.error(
                    "sendEventMessage",
                    err.message ?: "failed to mark the outbox message.", err
                )
            }
