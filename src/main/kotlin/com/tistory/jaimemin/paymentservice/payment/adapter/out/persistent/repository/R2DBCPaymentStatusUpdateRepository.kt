...
class R2DBCPaymentStatusUpdateRepository(
            ...
            .handle { paymentOrder, sink ->
                when (paymentOrder.second) {
                    ...
                    PaymentStatus.SUCCESS.name -> {
                        sink.error(
                            PaymentAlreadyProcessedException(
                                message = "이미 처리 성공한 결제입니다.", status = PaymentStatus.SUCCESS
                            )
                        )
                    }
                    ...
                    PaymentStatus.FAILURE.name -> {
                        sink.error(
                            PaymentAlreadyProcessedException(
                                message = "이미 처리 실패한 결제입니다.", status = PaymentStatus.FAILURE
                            )
                        )
                    }
                }
    ...
    private fun updatePaymentKey(orderId: String, key: String): Mono<Long> {
    ...
    private fun incrementPaymentOrderFailCount(command: PaymentStatusUpdateCommand): Mono<Long> {
